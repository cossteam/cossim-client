		...
		<Page className="calls-page">
			<Navbar className="calls-navbar" titleLarge="Calls" large transparent>
				...
				<Segmented slot="title" strong className="calls-navbar-segmented">
					<Button active={callsTab === 'all'} onClick={() => setCallsTab('all')}>
						All
					</Button>
					<Button active={callsTab === 'missed'} onClick={() => setCallsTab('missed')}>
						Missed
					</Button>
				</Segmented>
				<Link
			...
			<List noChevron mediaList dividers className="calls-list">
				{callsFormatted.map((call, index) => (
					...
					>
						...
						/>
						<span slot="title" className={call.type === 'Missed' ? 'text-color-red' : ''}>
							{call.contact.name}
						</span>
						...
						<SwipeoutActions right>
							<SwipeoutButton color="red" overswipe close onClick={() => onDeleteClick(call)}>
								Delete
							</SwipeoutButton>
						</SwipeoutActions>
					</ListItem>
