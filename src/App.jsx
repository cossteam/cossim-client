...
const AppComponent = () => {
    ...
    useEffect(() => {
        ...
        if ((f7.device.ios || f7.device.android) && f7.device.standalone) {
            ...
            viewEl.setAttribute(
                'content',
                `${viewEl.getAttribute('content')}, maximum-scale=1, user-scalable=no`
            )
        }
    }, [])
    ...
    return (
        <App {...f7params}>
            <Views tabs className="safe-areas bg-red-400">
                ...
                {/* <View id="view-settings" onTabShow={() => setActiveTab('settings')} tab url="/settings/" /> */}
            </Views>
        </App>
    )
}
