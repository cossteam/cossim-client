...
import { Icon, List, ListItem, Navbar, Link, Page, BlockTitle } from 'framework7-react'
import './Status.less'
		...
		<Page className="status-page">
			...
			<List mediaList noChevron strong dividers outline className="status-list">
				<ListItem title="My Status" text="Add to my status">
					<Link slot="media">
						<img src="/avatars/vladimir-kharlampidi.jpg" loading="lazy" alt="Vladimir Kharlampidi" />
			...
			<List mediaList noChevron strong dividers outline className="status-list">
				{statusesFormatted.map((status) => (
					<ListItem key={status.userId} title={status.contact.name} link text={status.hoursAgo}>
						<img
