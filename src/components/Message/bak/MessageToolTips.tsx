...
const MessageToolTips: React.FC<ToolTipProps> = ({ el, onSelect, parentEl }) => {
	...
	useEffect(() => {
		...
		const currentEl = parentEl?.current || null
		currentRef.current = currentEl
		if (currentEl && currentEl.parentElement) currentEl.parentElement.style.zIndex = '2'
	...
	useEffect(() => {
		!visible &&
			...
			(currentRef.current.parentElement.style.zIndex = '1')
	}, [visible])
...
export default MessageToolTips
