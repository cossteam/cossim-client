...
/**
 * 判断一个值是否为 ArrayBuffer 类型
 * @param thing 需要判断的值
 * @returns 如果是 ArrayBuffer 类型则返回 true，否则返回 false
 */
...
/**
 * 基于浏览器的 indexDB 实现的 SignalProtocolStore
 */
export class SignalProtocolStore implements StorageType {
	...
	get(key: string, defaultValue: StoreValue): StoreValue {
		if (!key) throw new Error('Tried to get value for undefined/null key')
...
/**
 * 将Uint8Array类型的数组转换为字符串
 * @param {Uint8Array} arr - 要转换的数组
