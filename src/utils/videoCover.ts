export default class VideoCover {
	url: any
	...
	constructor(
		config = {
			...
			isCheckImageColor: false
		}
	) {
		const { url, currentTime = 1, quality, imgWidth, imageType, isCheckImageColor } = config
	 ...
	 * @returns
	 */
	generateCanvas(callback: (img: string) => void) {
		if (!this.video) return
	 ...
	 */
	checkImage(ctx: CanvasRenderingContext2D | null, width: number, height: number) {
		if (!ctx) return true
	 ...
	 * @param {String} code base64字符串
	 */
	static downloadFile(code: string) {
		...
		aLink.download = String(fileName)
		aLink.href = URL.createObjectURL(blob ?? new Blob([]))
		aLink.click()
