...
const MentionComponent: React.FC<MentionProps> = (props) => {
	...
	return (
		<div className="mention-popup py-2" ref={MentionRef} data-active-index={activeIndex}>
			{!props.data?.length ? (
				<div className="text-center text-[0.9rem] text-gray-500">{$t('暂无数据')}</div>
			) : (
				<>
					<div
						className={clsx(
							'mention-popup-item py-[6px] px-2',
							-1 === activeIndex && 'bg-gray-200 text-[#184fff]'
						)}
						onClick={() => props.onSelect({ id: 0, name: $t('全体成员') })}
					>
						{$t('全体成员')}
					</div>
